...
def objective(trial: optuna.Trial) -> (float, float):
    ...
    try:
        early_stopping = int(params['num_boost_round'] * 0.15)
    except KeyError:
        early_stopping = 15
...
embedding_types = ["georgiev", "one_hot"]
m_types = ["lightgbm", "xgboost", "rf"]
cv_folds = 5
n_studies = 3  # number of study
n_trials = 150
