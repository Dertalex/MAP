...
starting_points_determination = "manual_setup"  
...
#mlde params
r_top = 0.97  #Percentage cutoff of top scoring datapoints, targeted to be identified during the MLDE CyclesS87F:Q95K:K112T:L198P
...
r_max_start = 0.3  # Decimal for percentage of maximum allowed score for the starting points 
...
benchmark_ID = f"{array_ID}-{dataset}_{repr_type}_{model_type}_nGain{n_gain}_nStart{n_starting_points}_r_max_start{int(r_max_start * 100)}_nCycles{n_cycles}_normedScores_{score_column}_run_{benchmark_run}"
...
for mut in mutations: #assuming the format is like S87F:Q95K:K112T:L198P and the dataset does not list the wildtype sequence
        index = int(mut[1:-1]) - 1
        wild_type = wild_type[:index] + mut[0] + wild_type[index + 1:]
max_score = max([isxyz[3] for isxyz in data])
max_starting_score = round(max_score * r_max_start, 3)
...
logging.info(f"")
logging.info(f"##################### MLDE-Parameters ####################")
...
logging.info(f"Percentage of maximum score, allowed for the Starting Points : {int(r_max_start*100)}%")
...
logging.info(f"")
logging.info(f"############ Hyper-Parameter-Tuning-Parameters ###########")
...
while not determined_starting_points and trial_counter < n_starting_point_trials:
    try:
        mlde_datapoints = random.sample([isxyz for isxyz in library if isxyz[3] <= r_max_start], int(n_starting_points))
...
logging.info(f"============================= MLDE-Benchmark-Start =============================\n")
...
# ood_performances = ["Spearman,NDCG,Pearson,R2,MSE"]  # Performance for each cycle's tested model the potential targets (mostly out of Distribution)
...
for i in range(1, n_cycles + 1):
    ...
    if update_params_after_each_cycle:
        mlde_optimizer = pop.Sequential_Optimizer(model_type=model_type, cv_folds=cv_folds, x_arr=x_mlde, y_arr=y_mlde,
                                                  ...
                                                  early_stopping_fraction=early_stopping_fraction, db_name = f"{benchmark_ID}_{i}")
