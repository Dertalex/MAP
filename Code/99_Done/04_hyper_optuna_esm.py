...
model = sys.argv[2]
cv_folds = 5
run_id = int(sys.argv[3])  # number of study
...
repr_layer = -1
random.seed = 42
path_to_embeddings = f"../Data/Embeddings/{dataset}/{e_type}/{repr_layer}"
mapping_file = f"../Data/Embeddings/{dataset}/pairings.csv"

with open(mapping_file) as f:
    data_pairs = [line[:-1] for line in f.readlines()[1:]]
    random.shuffle(data_pairs)

with tqdm.tqdm(total=len(data_pairs), desc="loading Embeddings from drive") as pbar:
    for line in data_pairs:
        line = line[:-1].split(',')
        embedding = f"{path_to_embeddings}/{line[0][1:]}"
        representation = torch.load(embedding,map_location=torch.device('cpu')).to(dtype=torch.float32).cpu().numpy()
        score = float(line[1])
        x_arr.append(representation)
        y_arr.append(score)
        pbar.update(1)
