...
to_encode = [line[1] for line in input_data[1:4]]
...
with (tqdm(total=len(to_encode)) as pbar):
    ...
    for i in range(0, len(to_encode), batch_size):
        with HiddenPrints():
            ...
            print(encodings[0])
            # encodings += ge.generate_sequence_encodings(e_type, to_encode)
            outpath = f"../../Data/Embeddings/{file_name}/{e_type}/batch_{i:0{pad}d}"
